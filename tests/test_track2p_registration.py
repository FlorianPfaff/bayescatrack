...
def test_register_plane_pair_track2p_transform_requires_backend(
    make_track2p_session,
    monkeypatch,
):
    masks = np.zeros((1, 4, 4), dtype=bool)
    masks[0, 1:3, 1:3] = True
    reference = make_track2p_session("2024-05-01_a", masks)
    moving = make_track2p_session("2024-05-02_a", masks)

    def _missing_backend():
        raise ImportError("missing test Track2p backend")

    monkeypatch.setattr(
        track2p_registration,
        "_load_track2p_registration_backend",
        _missing_backend,
    )

    with pytest.raises(
        ImportError,
        match="requires the Track2p/elastix registration backend",
    ):
        register_plane_pair(
...
def test_register_plane_pair_fov_translation_is_explicit_without_track2p_backend(
    make_track2p_session,
    monkeypatch,
):
    ...
    def _unexpected_backend():
        raise AssertionError("Track2p backend should not be loaded for fov-translation")
    ...
    assert registered.source.endswith("_fov_registered")
