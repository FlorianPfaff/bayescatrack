...
class CalibratedAssociationModel:
    ...
    def pairwise_cost_matrix_from_bundle(
        self, bundle: SessionAssociationBundle
    ) -> np.ndarray:
        return self.pairwise_cost_matrix_from_components(
            pairwise_components_from_bundle(bundle)
        )
...
def pairwise_components_from_bundle(
    bundle: SessionAssociationBundle, *, covariance_epsilon: float = 1.0e-6
) -> dict[str, np.ndarray]:
...
def collect_reference_training_examples(
    ...
    *,
    ...
    for session_a, session_b in session_edges:
        ...
        features = pairwise_feature_tensor(
            pairwise_components_from_bundle(bundle), feature_names=options.feature_names
        )
...
def calibrated_cost_matrix_from_bundle(
    bundle: SessionAssociationBundle, calibrated_model: CalibratedAssociationModel
) -> np.ndarray:
