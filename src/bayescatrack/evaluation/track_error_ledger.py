...
def track_error_ledger(
    ...
    *,
    ...
    predicted_rows = _predicted_track_rows(predicted_observations, reference_lookup, pairs)
    reference_rows = _reference_track_rows(reference_observations, predicted_lookup, pairs)
...
def _predicted_track_rows(
    ...
    for predicted_track_id, observations in enumerate(predicted_observations):
        reference_ids = [reference_lookup[observation] for observation in observations if observation in reference_lookup]
        reference_counts = Counter(reference_ids)
        unknown_observations = [observation for observation in observations if observation not in reference_lookup]
        false_links = _predicted_track_false_links(observations, reference_lookup, session_pairs)
        identity_switches = _identity_switches(observations, reference_lookup)
        rows.append(
            {
                "track_id": int(predicted_track_id),
                "category": _predicted_track_category(reference_counts, unknown_observations, identity_switches),
                "length": len(observations),
                "reference_track_ids": sorted(int(track_id) for track_id in reference_counts),
...
def _reference_track_rows(
    ...
    for reference_track_id, observations in enumerate(reference_observations):
        predicted_ids = [predicted_lookup[observation] for observation in observations if observation in predicted_lookup]
        predicted_counts = Counter(predicted_ids)
        missed_observations = [observation for observation in observations if observation not in predicted_lookup]
        missed_links = _reference_track_missed_links(observations, predicted_lookup, session_pairs)
        rows.append(
            {
                "track_id": int(reference_track_id),
                "category": _reference_track_category(predicted_counts, missed_observations),
                "length": len(observations),
                "predicted_track_ids": sorted(int(track_id) for track_id in predicted_counts),
...
def _summary_rows(
    ...
    *,
    ...
    fragmented_reference_tracks = sum(1 for row in reference_rows if row["fragment_count"] > 0)
    missed_reference_tracks = sum(1 for row in reference_rows if row["category"] == "missed")
    partial_reference_tracks = sum(1 for row in reference_rows if row["category"] in {"partial", "fragmented_partial"})
    mixed_identity_tracks = sum(1 for row in predicted_rows if row["category"] == "mixed_identity")
    ...
    return {
        ...
        "false_continuation_rate": _safe_error_rate(false_continuation_links, len(predicted_links)),
        "missed_reference_links": int(missed_reference_links),
        "missed_reference_link_rate": _safe_error_rate(missed_reference_links, len(reference_links)),
        ...
        "track_fragmentation_rate": _safe_error_rate(fragmented_reference_tracks, reference_tracks),
        "missed_reference_tracks": int(missed_reference_tracks),
        "missed_reference_track_rate": _safe_error_rate(missed_reference_tracks, reference_tracks),
        ...
        "predicted_tracks_with_errors": int(sum(1 for row in predicted_rows if row["category"] != "single_identity")),
        "reference_tracks_with_errors": int(sum(1 for row in reference_rows if row["category"] != "recovered")),
        "predicted_track_error_rate": _safe_error_rate(sum(1 for row in predicted_rows if row["category"] != "single_identity"), predicted_tracks),
        "reference_track_error_rate": _safe_error_rate(sum(1 for row in reference_rows if row["category"] != "recovered"), reference_tracks),
    }
...
def _false_continuation_row(
    ...
    return {
        ...
        "roi_a": int(roi_a),
        "roi_b": int(roi_b),
        "predicted_track_id": _same_or_none(predicted_lookup.get(obs_a), predicted_lookup.get(obs_b)),
...
def _missed_reference_link_row(
    ...
    return {
        ...
        "roi_a": int(roi_a),
        "roi_b": int(roi_b),
        "reference_track_id": _same_or_none(reference_lookup.get(obs_a), reference_lookup.get(obs_b)),
...
def _identity_switches(observations: list[Observation], reference_lookup: dict[Observation, int]) -> int:
...
def _observation_lookup(observations_by_track: list[list[Observation]]) -> tuple[dict[Observation, int], list[tuple[Observation, int, int]]]:
...
def _session_pairs(matrix: np.ndarray, session_pairs: Iterable[tuple[int, int]] | None) -> tuple[tuple[int, int], ...]:
    if session_pairs is None:
        return tuple((session_idx, session_idx + 1) for session_idx in range(max(0, matrix.shape[1] - 1)))
    pairs = tuple((int(session_a), int(session_b)) for session_a, session_b in session_pairs)
    for session_a, session_b in pairs:
        if session_a < 0 or session_b < 0 or session_a >= matrix.shape[1] or session_b >= matrix.shape[1]:
            raise IndexError(f"session pair {(session_a, session_b)!r} is out of bounds for {matrix.shape[1]} sessions")
